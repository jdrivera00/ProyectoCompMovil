...
import { Feather } from '@expo/vector-icons'; // Importa el icono de la papelera
...
export default function BorrarProducto() {
  ...
  const handleBorrarProducto = (id) => {
    router.push(`/admin/BorrandoProducto?id=${id}`);
  };
  ...
  const renderItem = ({ item }) => (
    <View style={styles.menuItem}>
      ...
      <TouchableOpacity onPress={() => handleBorrarProducto(item.id)} style={styles.deleteButton}>
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Borrar Productos</Text>
      <FlatList
        data={menuItems}
...
const styles = StyleSheet.create({
  ...
  menuItem: {
    ...
    elevation: 2,
    alignItems: 'center',
    justifyContent: 'space-between', // Para alinear el botón de borrar a la derecha
  },
  itemImage: {
    width: 80,
    height: 80,
